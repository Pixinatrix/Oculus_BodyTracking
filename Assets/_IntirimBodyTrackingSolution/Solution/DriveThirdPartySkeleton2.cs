...
namespace Oculus.Movement.Tracking
{
    ...
    /// Identical to <see cref="DriveThirdPartySkeleton"/>, except for
    /// <see cref="Start"/> and <see cref="Reset"/> methods.
    /// </summary>
    [DefaultExecutionOrder(-60)]
    public class DriveThirdPartySkeleton2 : MonoBehaviour
    {
#if UNITY_EDITOR
        /// <summary>
        /// convenience to copy from old DriveThirdPartySkeleton class
        /// </summary>
        void Reset() {
            DriveThirdPartySkeleton d3rdP = GetComponent<DriveThirdPartySkeleton>();
            if (d3rdP == null) { return; }
            Type t = typeof(DriveThirdPartySkeleton);
            System.Reflection.BindingFlags priv = System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance;
            T Other<T>(string name) where T : class {
                System.Reflection.FieldInfo f = t.GetField(name, priv);
                return f.GetValue(d3rdP) as T;
            }
            _animatorTargetSkeleton = Other<Animator>(nameof(_animatorTargetSkeleton));
            _animatorTargetTPose = Other<Animator>(nameof(_animatorTargetTPose));
            _ovrSkeleton = Other<OVRSkeleton>(nameof(_ovrSkeleton));
            _lineRendererSourcePrefab = Other<GameObject>(nameof(_lineRendererSourcePrefab));
            _lineRendererTargetPrefab = Other<GameObject>(nameof(_lineRendererTargetPrefab));
            _jointRendererSourcePrefab = Other<GameObject>(nameof(_jointRendererSourcePrefab));
            _jointRendererTargetPrefab = Other<GameObject>(nameof(_jointRendererTargetPrefab));
            _axisRendererSourcePrefab = Other<GameObject>(nameof(_axisRendererSourcePrefab));
            _axisRendererTargetPrefab = Other<GameObject>(nameof(_axisRendererTargetPrefab));
            _targetRenderer = Other<Renderer>(nameof(_targetRenderer));
        }
#endif
        ...
        public class JointAdjustment : ISerializationCallbackReceiver
        {
            public void OnBeforeSerialize()
            {
                ...
                if (!_hasBeenSerialized)
                {
                    ...
                    BoneIdOverrideValue = HumanBodyBonesMappings.BodyTrackingBoneId.NoOverride;
                }
            ...
            public HumanBodyBonesMappings.BodyTrackingBoneId BoneIdOverrideValue = HumanBodyBonesMappings.BodyTrackingBoneId.NoOverride;
        }
        ...
        protected HumanBodyBonesMappings.BodySection[] _bodySectionsToAlign =
        {
            HumanBodyBonesMappings.BodySection.LeftArm, HumanBodyBonesMappings.BodySection.RightArm,
            HumanBodyBonesMappings.BodySection.LeftHand, HumanBodyBonesMappings.BodySection.RightHand,
            HumanBodyBonesMappings.BodySection.Hips, HumanBodyBonesMappings.BodySection.Back,
            HumanBodyBonesMappings.BodySection.Neck, HumanBodyBonesMappings.BodySection.Head
        };
        ...
        protected HumanBodyBonesMappings.BodySection[] _bodySectionToPosition =
        {
            HumanBodyBonesMappings.BodySection.LeftArm, HumanBodyBonesMappings.BodySection.RightArm,
            HumanBodyBonesMappings.BodySection.LeftHand, HumanBodyBonesMappings.BodySection.RightHand,
            HumanBodyBonesMappings.BodySection.Hips, HumanBodyBonesMappings.BodySection.Neck,
            HumanBodyBonesMappings.BodySection.Head
        };
        ...
        protected HumanBodyBonesMappings.BodySection[] _bodySectionsToRenderDebugText =
        {
            HumanBodyBonesMappings.BodySection.Back, HumanBodyBonesMappings.BodySection.Hips,
            HumanBodyBonesMappings.BodySection.Neck, HumanBodyBonesMappings.BodySection.Head,
            HumanBodyBonesMappings.BodySection.LeftLeg, HumanBodyBonesMappings.BodySection.RightLeg,
            HumanBodyBonesMappings.BodySection.LeftArm, HumanBodyBonesMappings.BodySection.RightArm
        };
        ...
        virtual protected void Start()
        {
            ...
            if (_animatorTargetSkeleton == _animatorTargetTPose) {
                _animatorTargetTPose = CreateTPoseAnimatorCopy();
            }

            if (_animatorTargetTPose != null) {
        ...
        /// <summary>
        /// creates a mostly inert copy to be used as a T-Pose skeleton
        /// </summary>
        private Animator CreateTPoseAnimatorCopy() {
            GameObject tPose = Instantiate(_animatorTargetSkeleton.gameObject);
            Behaviour[] behaviors = tPose.GetComponentsInChildren<Behaviour>();
            // disable components that aren't the DriveThirdPartySkeleton
            foreach(Behaviour b in behaviors) {
                if (b is DriveThirdPartySkeleton2 drive3rdPartySkeleton) {
                    SpecialDisableLogicFor(drive3rdPartySkeleton);
                    continue;
                }
                b.enabled = false;
            }
            tPose.name += "(T Pose Copy)";
            return tPose.GetComponent<Animator>();
        }

        private void SpecialDisableLogicFor(DriveThirdPartySkeleton2 d3s) {
            d3s.gameObject.AddComponent<FinishDisableAfterStart>();
            foreach(Transform child in d3s.transform) {
                child.gameObject.SetActive(false);
            }
            // prevent T-pose from doing more T-pose logic
            d3s._animatorTargetTPose = null;
        }

        /// <summary>
        /// highly specialized class used as a callback. deactivates behaviors.
        /// entire GameObject can't be deactivated, Animator needs it.
        /// </summary>
        private class FinishDisableAfterStart : MonoBehaviour {
            private void Update() {
                GetComponent<DriveThirdPartySkeleton2>().enabled = false;
                enabled = false;
            }
        }
        ...
        private void AdjustCustomBoneIdToHumanBodyBoneMapping()
        {
            ...
            {
                if (adjustment.BoneIdOverrideValue == HumanBodyBonesMappings.BodyTrackingBoneId.NoOverride)
                {
                    continue;
                }

                if (adjustment.BoneIdOverrideValue == HumanBodyBonesMappings.BodyTrackingBoneId.Remove)
                {
        ...
        private void ComputeOffsetsUsingSkeletonComponent()
        {
            ...
            for (int i = 0; i < numBones; i++)
            {
                ...
                var skelBoneId = (OVRSkeleton.BoneId)currBindPose.Id;
                ...
                var bodySection = HumanBodyBonesMappings.BoneToBodySection[humanBodyBone];
        ...
        private void DrawGizmosForBothBoneDatasSource(
            Dictionary<HumanBodyBones, SkeletonMetadata.BoneData> boneToDataSource,
        ...
        private void DrawGizmosForBothBoneDatasTarget(
            Dictionary<HumanBodyBones, SkeletonMetadata.BoneData> boneToDataTarget,
        ...
        private void DrawJointPair(HumanBodyBones bodyBone, SkeletonMetadata.BoneData boneData, string prefix,
            ...
            bool print = false)
        {
            ...
            foreach (var key in boneDataMap.Keys)
            {
                ...
                if (renderObject.AnnotatedLineRenderers[lineIndex] == null)
                {
                    ...
                    var bodySection = HumanBodyBonesMappings.BoneToBodySection[key];
        ...
        private void AlignTargetWithSource()
        {
            ...
            for (int i = 0; i < numBones; i++)
            {
                ...
                var bodySectionOfJoint = HumanBodyBonesMappings.BoneToBodySection[humanBodyBone];
        ...
        private bool IsBodySectionInArray(
            HumanBodyBonesMappings.BodySection bodySectionToCheck,
            HumanBodyBonesMappings.BodySection[] sectionArrayToCheck)
        {
